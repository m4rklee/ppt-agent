...
class PPTGen(ABC):
    ...
    def _collect_history(self, code_executor: CodeExecutor):
        ...
        for role_name, role in self.staffs.items():
            history["agents"][role_name] = [
                turn.to_dict() for turn in role.history
            ]
            role._history = []

        return history

    def _staff_history_lens(self) -> dict[str, int]:
        return {name: len(role._history) for name, role in self.staffs.items()}

    def _slice_staff_history(self, lens_before: dict[str, int]) -> dict[str, list]:
        agents: dict[str, list] = {}
        for name, role in self.staffs.items():
            start = lens_before.get(name, 0)
            new_turns = role.history[start:]
            if new_turns:
                agents[name] = [turn.to_dict() for turn in new_turns]
        return agents
...
class PPTGenAsync(PPTGen):
    ...
    async def generate_pres(
        ...
        async def notify(event: str, data: dict[str, Any]):
            ...
            return await on_progress(event, payload)

        if outline is None:
            self.outline = await self.generate_outline(num_slides, source_doc)
        else:
            self.outline = outline
        reviewed_outline = await notify(
            "outline_done",
            {
                "outline": [
                    {
        ...
        if reviewed_outline is not None:
            for slide_idx, edited_item in enumerate(reviewed_outline):
                if slide_idx >= len(self.outline) or not isinstance(edited_item, dict):
                    continue
                purpose = edited_item.get("purpose")
                section = edited_item.get("section")
                if isinstance(purpose, str) and purpose.strip():
                    self.outline[slide_idx].purpose = purpose.strip()
                if isinstance(section, str) and section.strip():
                    self.outline[slide_idx].section = section.strip()
        self.simple_outline = "\n".join(
            [
                f"Slide {slide_idx+1}: {item.purpose}"
                for slide_idx, item in enumerate(self.outline)
        ...
        async def generate_slide_wrapped(
            ...
            try:
                ...
                per_slide_traces.append(slide_trace)
                await notify("agent_turn", slide_trace)
                return slide_idx, outline_item, None, e

        slide_tasks = []
        for slide_idx, outline_item in enumerate(self.outline):
            if self.force_pages and slide_idx == num_slides:
                break
            slide_tasks.append(generate_slide_wrapped(slide_idx, outline_item))

        total_slides = len(slide_tasks)
        slide_results_by_idx: dict[int, tuple] = {}
        completed = 0
        for finished in asyncio.as_completed(slide_tasks):
            slide_idx, outline_item, result, error = await finished
            completed += 1
        ...
        for slide_idx in range(total_slides):
            result, error = slide_results_by_idx[slide_idx]
        ...
        history["per_slide"] = sorted(per_slide_traces, key=lambda x: x["index"])
